...
const cpuData = Array.from({ length: 24 }, (_, i) => ({
  ...
  cpu: Math.round(20 + Math.sin(i / 2) * 14 + Math.random() * 12),
}))

const ramData = [
  { name: "System", value: 3.1 },
  { name: "Containers", value: 8.4 },
  { name: "Cache", value: 2.6 },
  { name: "Free", value: 1.9 },
]

const netData = Array.from({ length: 20 }, (_, i) => ({
  t: `${i * 3}m`,
  down: Math.round(400 + Math.sin(i / 1.5) * 300 + Math.random() * 200),
  up: Math.round(120 + Math.cos(i / 2) * 90 + Math.random() * 70),
}))
  ...
  return (
    <div className="space-y-6 p-8">
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        <ChartCard title="CPU Usage" subtitle="Last 24 hours · %" icon={Cpu}>
          <ResponsiveContainer width="100%" height={240}>
            <AreaChart data={cpuData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
              ...
              <XAxis dataKey="t" stroke="#8e8e93" fontSize={11} tickLine={false} axisLine={false} interval={5} />
              ...
              <Area type="monotone" dataKey="cpu" stroke={CHART_COLORS.cpu} strokeWidth={2} fill="url(#cpuFill)" />
            </AreaChart>
          </ResponsiveContainer>
        </ChartCard>

        <ChartCard title="Memory Allocation" subtitle="16 GB total · GB per pool" icon={MemoryStick}>
          <ResponsiveContainer width="100%" height={240}>
      ...
      <ChartCard title="Network Throughput" subtitle="Download / Upload · KB/s" icon={Network}>
        <ResponsiveContainer width="100%" height={260}>
          <LineChart data={netData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
            <CartesianGrid strokeDasharray="3 3" stroke="rgba(255,255,255,0.06)" vertical={false} />
            <XAxis dataKey="t" stroke="#8e8e93" fontSize={11} tickLine={false} axisLine={false} />
            <YAxis stroke="#8e8e93" fontSize={11} tickLine={false} axisLine={false} />
            <Tooltip contentStyle={tooltipStyle} />
            <Line type="monotone" dataKey="down" stroke={CHART_COLORS.network} strokeWidth={2} dot={false} name="Download" />
            <Line type="monotone" dataKey="up" stroke={CHART_COLORS.upload} strokeWidth={2} dot={false} name="Upload" />
          </LineChart>
        </ResponsiveContainer>
