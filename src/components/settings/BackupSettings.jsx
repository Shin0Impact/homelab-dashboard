...
export function BackupSettings({ isAdmin = true }) {
    ...
    <div className={`rounded-2xl p-6 ${glass} lg:col-span-2`}>
      ...
      <div className="mt-4 flex flex-wrap gap-3">
        ...
        <label
          ...
          className={`flex items-center gap-2 rounded-lg border border-white/10 bg-secondary/50 px-4 py-2 text-xs font-medium ${
            isAdmin ? "cursor-pointer hover:bg-secondary" : "cursor-not-allowed opacity-40"
          }`}
        >
          <Upload className="h-3.5 w-3.5" /> Import Settings & Stacks
          <input type="file" accept=".json" onChange={handleImport} disabled={!isAdmin} className="hidden" />
        </label>
        ...
        <button
          ...
          disabled={!isAdmin}
          title={!isAdmin ? "Admin access required" : undefined}
          className="flex items-center gap-2 rounded-lg border border-destructive/20 bg-destructive/10 px-4 py-2 text-xs font-medium text-destructive hover:bg-destructive/20 disabled:cursor-not-allowed disabled:opacity-40"
        >
